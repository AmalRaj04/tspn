...
- **Phase 9 (Experiments and Training Runs)**: **In progress** as of 2026-08-14. Measured cost made the original fixed run order (Leontief → VAR → MLP → GCN ablation → no-temporal ablation → no-shock ablation → 1-layer ablation → full TSPN, all at `max_epochs=200`) infeasible to run to completion in one sitting on this CPU-only machine (no usable GPU — see finding #31); MPS acceleration was investigated and ruled out, not just assumed unavailable. User chose to scope Phase 9 as: full TSPN (all 6 LOEO folds, real 200-epoch budget) first, ablations after. Leontief/VAR/MLP are already done (Phase 6). Added resume-from-checkpoint to `train.py` before launching (Locked Plan §9.2's own guidance, not optional given the multi-hour-to-multi-day runtime) — caught and fixed a real bug in my first implementation via testing (a frozen `done` flag incorrectly blocked resuming past a prior run's smaller epoch cap; fixed by evaluating completion fresh against the current call's `max_epochs` each time, see finding #32). Full TSPN training launched in the background; see this file's next update for results once complete.
