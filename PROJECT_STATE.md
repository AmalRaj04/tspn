...
- **Phase 9 ablations (GCN, no-temporal, no-shock, 1-layer) through Phase 13 (Maintenance)**: Not started. Watch CP37 (re-initialize model + optimizer inside each ablation loop iteration) when building these. See the approved plan for the full Phase 9–13 outline.

### Phase 9 deliverables so far (this session)
- `src/training/train.py`: added resume-from-checkpoint capability (per-fold JSON sidecar: epoch, best_val_rmse, patience_counter, early_stopped; fresh optimizer/scheduler on resume per §9.2; idempotent re-invocation)
