...
- **Phase 9 (Experiments and Training Runs) through Phase 13 (Maintenance)**: Not started. Phase 9 runs the fixed experiment order (Leontief → VAR → MLP → GCN ablation → no-temporal ablation → no-shock ablation → 1-layer ablation → full TSPN) at the real `max_epochs=200` budget per fold — this will take substantially longer than the 1-epoch smoke test (~9.5 min/epoch/fold observed on this CPU-only setup, so a full 6-fold x 200-epoch TSPN run alone is on the order of many hours; ablations add more). Watch CP37 (re-initialize model + optimizer inside each ablation loop iteration). See the approved plan for the full Phase 9–13 outline.
