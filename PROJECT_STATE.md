...
- **Phase 8 (Training Infrastructure) through Phase 13 (Maintenance)**: Not started. Immediate next step is `src/training/losses.py`, `augmentation.py`, `train.py` (6-fold LOEO-CV), `evaluate.py`. Watch CP33 (hard-fail contamination assertion every fold) and CP34 (augmentation must never run during validation). See the approved plan for the full Phase 8–13 outline.
